...
{
    QString ret = QString("coinonat:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CXT unit input in lnReqAmount, we generate the URI with CXT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(coinonatUnits::format(coinonatUnits::CXT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
